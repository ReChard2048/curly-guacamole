...
- **搜索框**（右上角）：按名字过滤知识点，匹配的节点连着它的层级一路显示出来，搜不到会提示。清空就恢复整棵树。
- **分类节点可折叠**：带 ▾ 的「分类」行点一下折叠/展开子树（搜索状态下折叠暂时忽略，直接显示匹配链）。
...
> `LICENSE` 里只放官方条款原文，版权声明和联系方式写在这儿就行。GitHub 的协议识别**不收录 PolyForm 系列**，所以仓库页面上会把它显示成 "Other" —— 这是正常的，条款本身照样有效，以文件内容为准。
